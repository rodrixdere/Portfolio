...
export default function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    function setSize() {
      // Escalar por DPR para render nítido en pantallas retina;
      // la física sigue trabajando en píxeles CSS
          ...
          for (let x = STEP; x < W; x += STEP) {
            ...
            if (Math.random() < 0.12) {
              particles.current.push({
                ox: x, oy: y,
                x: reduceMotion ? x : x + (Math.random() - 0.5) * W * 0.5,
                y: reduceMotion ? y : y + (Math.random() - 0.5) * H * 0.5,
                vx: 0, vy: 0,
      ...
      for (let y = 0; y < offH; y += STEP) {
        ...
        for (let x = 0; x < offW; x += STEP) {
          ...
          if (brightness > 28 && Math.random() < 0.65) {
            ...
            particles.current.push({
              ox: jx, oy: jy,
              x: reduceMotion ? jx : jx + (Math.random() - 0.5) * W * 0.6,
              y: reduceMotion ? jy : jy + (Math.random() - 0.5) * H * 0.6,
              vx: 0, vy: 0,
    ...
    // Batched render — agrupa por alpha, ~10 draw calls en vez de N
    function draw() {
